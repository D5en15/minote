...
**Refactoring & Enhancement Phase**

**Current Task:** Bugfix - note CRUD failure investigation and recovery UX

หมายเหตุ: รอบนี้หยุดงาน enhancement ชั่วคราวเพื่อแก้ regression ที่ผู้ใช้รายงานว่าเพิ่ม/ลบ/แก้ไขโน้ตไม่ได้ โดยตรวจ verifier ของ notes API ซ้ำ, ไล่ flow ฝั่ง note workspace และเพิ่มการแสดงผล error ของ create/delete/autosave ให้ผู้ใช้เห็นสาเหตุจริงแทนการ fail แบบเงียบ
...
* เพิ่ม migration `supabase/migrations/20260707000100_subscription_tier_foundation.sql` เพื่อขยาย `profiles` ด้วย `tier` และ Stripe snapshot fields และขยาย `plans` ให้รองรับ metadata ของ Zen Free, Zen Pro และ Zen Studio
* เพิ่ม `src/server/billing/plans.ts` สำหรับ map Stripe price id -> canonical plan id และ derive tier (`free`, `pro`, `studio`) แบบรวมศูนย์
* ปรับ `src/app/api/webhooks/stripe/route.ts` ให้ map Zen Studio ไป plan ของตัวเองจริง, รองรับสถานะ `unpaid`, และ sync `profiles.tier`, `stripe_customer_id`, `stripe_subscription_status`, `current_period_end` ทุกครั้งที่ webhook อัปเดต subscription
* ปรับ `src/types/database.ts` ให้สะท้อน plan tier, billing interval และ fields ใหม่ของ subscription foundation
* เพิ่ม migration `supabase/migrations/20260707000200_share_settings_and_plan_enforcement.sql` สำหรับ persist share font/branding/theme/metadata settings ต่อ link
* เพิ่ม `src/server/features.ts` และ `src/server/errors.ts` เพื่อรวม feature gating ตาม plan และ quota error shape (`QUOTA_EXCEEDED`) ไว้ใช้ร่วมกัน
* ปรับ `src/server/quota.ts`, `src/server/services/notes.ts`, `src/server/services/tags.ts` และ notes/tags API routes ให้ enforce Free limits ตาม pricing matrix จริง: 3 notes/day, 50 total notes, 3 tags/note
* ปรับ `src/server/repositories/shares.ts`, `src/server/services/shares.ts`, share API routes และ `src/components/notes/note-workspace.tsx` ให้รองรับ share settings (`fontFamily`, `showBranding`, `showThemeToggle`, `showCreatedAt`) พร้อม backend gating ตาม plan
* ปรับ `src/components/share/shared-note-view.tsx`, `src/components/share/share-theme-toggle.tsx` และ `src/app/share/[token]/page.tsx` ให้ shared page แสดง Poppins/Lora, watermark และ metadata ตาม setting ของ link
* ปรับ `src/server/services/billing.ts` และ `src/app/app/billing/billing-client-page.tsx` ให้แสดง tier limits/feature flags และ grace period warning
* รัน `npx tsc --noEmit`, targeted eslint สำหรับไฟล์ที่แก้ และ `npm run build` ผ่านหลังจบรอบ enhancement นี้
* ปรับ `src/components/guest/guest-workspace.tsx` จาก 2-column pricing cards เป็น 3-tier feature comparison matrix พร้อม monthly/yearly toggle, yearly savings label และ CTA row ด้านล่างของแต่ละ plan
* ปรับ `src/components/auth/login-panel.tsx` ให้รับ `checkoutPriceId` จาก CTA ของหน้า public และ persist intended Stripe price id ไว้ใน localStorage ก่อนเริ่ม auth flow
* รัน `npx tsc --noEmit`, targeted eslint สำหรับ `guest-workspace`/`login-panel` และ `npm run build` ผ่านหลังจบ landing page pricing refactor
* ปรับ `src/app/app/billing/billing-client-page.tsx` ให้ใช้ billing-cycle toggle เดียวด้านบน section, เปลี่ยนราคา/ข้อความ/ปุ่ม upgrade ของ Zen Pro และ Zen Studio ตาม state `isYearly`, และตัด stacked monthly/yearly buttons เดิมออก
* ปรับ `src/components/guest/guest-workspace.tsx` อีกครั้งจาก comparison matrix เป็น 3-column pricing cards แบบ responsive โดยคง toggle กลาง section, dynamic CTA labels และ checkout price-id switching ตาม billing cycle เดิม
* รัน `node scripts/verify-phase5.mjs` ซ้ำและยืนยันว่า notes API create/list/get/update/delete/restore/permanent delete ผ่านแบบ end-to-end
* ปรับ `src/components/notes/use-debounced-autosave.ts` และ `src/components/notes/note-workspace.tsx` ให้แสดงข้อความ error ชัดเจนสำหรับ create/delete/autosave failures เช่น session หมดอายุ, quota เต็ม หรือ internal error แทนการกลืน failure เงียบ
...
| `scripts/verify-phase16.mjs` | Script สำหรับตรวจ global security headers, Content-Security-Policy (CSP) rules, XSS protection, and frame blockages |
| `scripts/verify-phase17.mjs` | Script สำหรับตรวจ database health check, custom error severity logger metrics, and dynamic error tracking integrations |
| `scripts/verify-phase18.mjs` | Script สำหรับตรวจ standalone unit test coverage, note lifecycle CRUD operations, and E2E endpoints validation |
| `supabase/migrations/20260707000100_subscription_tier_foundation.sql` | Migration สำหรับเพิ่ม tier-aware subscription foundation ใน `profiles` และ `plans` |
| `supabase/migrations/20260707000200_share_settings_and_plan_enforcement.sql` | Migration สำหรับเพิ่ม persisted share settings และ plan enforcement fields ที่ใช้งานจริงใน runtime |
| `src/components/guest/guest-workspace.tsx` | Landing page UI พร้อม hero, feature section และ 3-column pricing cards ที่ใช้ monthly/yearly toggle และ CTA mapping เข้า auth flow |
| `src/components/notes/note-workspace.tsx` | Notes workspace UI ที่เพิ่มการแสดงผล error สำหรับ create/delete/load note failures |
| `src/components/notes/use-debounced-autosave.ts` | Autosave hook ที่ส่ง error message กลับไปยัง editor UI เมื่อ save ล้มเหลว |
| `src/components/auth/login-panel.tsx` | Login panel ที่รองรับการเก็บ intended checkout price id จาก CTA บนหน้า public |
| `src/components/notes/note-workspace.tsx` | Cloud Workspace สำหรับจัดการ Notes ที่ลบการแจ้งเตือน Import guest notes ออก |
| `src/app/app/billing/billing-client-page.tsx` | UI สำหรับแสดงข้อมูลแผนการสมัครสมาชิกพร้อม billing-cycle toggle กลาง section และปุ่ม checkout ที่เปลี่ยนตาม Monthly/Yearly |
| `src/app/api/webhooks/stripe/route.ts` | Stripe webhook handling mapping logic for pricing products |
| `src/server/billing/plans.ts` | Canonical subscription plan/tier mapping helper สำหรับ webhook และ feature gating รอบถัดไป |
| `src/server/features.ts` | Feature access helper กลางสำหรับอ่านสิทธิ์จาก quota entitlement |
| `src/server/errors.ts` | Shared domain errors โดยเฉพาะ `QUOTA_EXCEEDED` สำหรับ enforcement APIs |
