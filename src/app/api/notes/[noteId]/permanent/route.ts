...
import {
  assertNoteOwner,
  permanentlyDeleteUserNote,
} from "@/server/services/notes";

type PermanentRouteContext = {
  params: Promise<{
    noteId: string;
  }>;
};
...
export async function DELETE(
  ...
  try {
    ...
    const note = await permanentlyDeleteUserNote(
      params.data.noteId,
      user.id,
      request
    );
    ...
    return errorResponse("INTERNAL_ERROR", "Unable to permanently delete note");
  }
}
